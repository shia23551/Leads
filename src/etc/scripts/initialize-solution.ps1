...
$jobs += Start-Job -Name "DbMigrator" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/Leads.DbMigrator")
    dotnet run
    dotnet run
...
$jobs += Start-Job -Name "DevCert" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/Leads.Web")
    dotnet dev-certs https -v -ep openiddict.pfx -p ed701279-0806-4b89-824d-d3c0e4d941db
